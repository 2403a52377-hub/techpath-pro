...
function LearnHub() {
  ...
  return (
    <AppShell>
      <PageHeader title="Learning Hub" subtitle="Tutorials, cheat sheets, notes and curated YouTube courses for every roadmap skill." />
      ...
      <div className="mt-6 grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
        {skills.map((skill) => (
          <a key={skill.name} href={skill.youtube} target="_blank" rel="noreferrer"
            ...
            <div className="aspect-video bg-gradient-primary relative grid place-items-center">
              ...
              <span className="absolute bottom-2 right-2 text-xs bg-background/90 text-foreground px-2 py-0.5 rounded">YouTube</span>
            </div>
            <div className="p-4">
              <p className="font-semibold">{skill.name}</p>
              <p className="text-xs text-muted-foreground mt-1 flex items-center gap-1">Top-rated course <ExternalLink className="size-3" /></p>
              <div className="mt-3 flex gap-2">
                <span className="text-xs px-2 py-0.5 rounded-full bg-secondary/20 text-secondary-foreground flex items-center gap-1"><FileText className="size-3" />Notes</span>
                <span className="text-xs px-2 py-0.5 rounded-full bg-accent/20 text-accent flex items-center gap-1"><BookOpen className="size-3" />Cheat sheet</span>
              </div>
            </div>
          </a>
        ))}
