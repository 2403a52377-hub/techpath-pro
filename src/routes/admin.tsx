...
interface JobRow { id: string; title: string; company: string; location: string | null; created_at: string }
interface RoadmapRow { id: string; title: string; slug: string; difficulty: string | null }
...
function AdminPage() {
  ...
  const [newJob, setNewJob] = useState({ title: "", company: "", location: "", type: "Internship", url: "" });
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const [u, r, j, p, jl, rl] = await Promise.all([
        ...
        supabase.from("jobs").select("id,title,company,location,created_at").order("created_at", { ascending: false }).limit(10),
        supabase.from("roadmaps").select("id,title,slug,difficulty").order("title").limit(20),
      ]);
      setCounts({ users: u.count ?? 0, roadmaps: r.count ?? 0, jobs: j.count ?? 0, posts: p.count ?? 0 });
      setJobs((jl.data as JobRow[]) ?? []);
      setRoadmaps((rl.data as RoadmapRow[]) ?? []);
    })();
  }, [user?.id, loading]);

  async function addJob() {
    if (!newJob.title || !newJob.company) { toast.error("Title and company required"); return; }
    const { error } = await supabase.from("jobs").insert({
      title: newJob.title.trim(),
      company: newJob.company.trim(),
      location: newJob.location.trim() || null,
      job_type: newJob.type,
      apply_url: newJob.url.trim() || null,
    } as never);
    if (error) { toast.error(error.message); return; }
    toast.success("Job posted");
    setNewJob({ title: "", company: "", location: "", type: "Internship", url: "" });
    const { data } = await supabase.from("jobs").select("id,title,company,location,created_at").order("created_at", { ascending: false }).limit(10);
      ...
      <AppShell>
        <div className="glass-card rounded-2xl p-10 text-center max-w-md mx-auto">
          ...
          <p className="mt-2 text-muted-foreground text-sm">
            This area is restricted to TechLand administrators.
          </p>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <div className="glass-card rounded-2xl p-6">
          ...
          <div className="space-y-3">
            <input className="w-full px-3 py-2 rounded-lg bg-background/60 border border-border" placeholder="Title (e.g. SDE Intern)"
            ...
            <input className="w-full px-3 py-2 rounded-lg bg-background/60 border border-border" placeholder="Apply URL"
              value={newJob.url} onChange={(e) => setNewJob({ ...newJob, url: e.target.value })} />
