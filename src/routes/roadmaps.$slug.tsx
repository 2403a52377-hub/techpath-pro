...
function RoadmapDetail() {
    ...
    return (
      <AppShell>
        <p className="text-center py-20 text-muted-foreground">
          Roadmap not found.{" "}
          <Link to="/roadmaps" className="text-primary underline">
            Back
          </Link>
        </p>
      </AppShell>
  ...
  return (
    <AppShell>
      <PageHeader
        title={roadmap.title}
        subtitle={`${roadmap.description} • ${roadmap.estimated_duration} • ${modules.length} modules`}
      />
      ...
      <div className="mt-6 grid md:grid-cols-2 gap-5">
        {modules.map((m, i) => {
          ...
          return (
            <div
              key={m.id}
              className="glass-card rounded-2xl overflow-hidden hover:shadow-elegant transition-all"
            >
              {m.youtube_thumbnail && (
                <a
                  href={m.youtube_url ?? "#"}
      ...
      <div className="mt-8">
        <Link to="/roadmaps" className="text-sm text-accent hover:underline">
          ← All roadmaps
        </Link>
      </div>
    </AppShell>
