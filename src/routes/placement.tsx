...
const SECTIONS = [
  {
    ...
    topics: ["Percentages", "Profit & Loss", "Time & Work", "Probability"],
  },
  {
    icon: Brain,
    title: "Logical Reasoning",
    topics: ["Series", "Puzzles", "Syllogism", "Blood Relations"],
  },
  {
    icon: MessageSquareText,
    title: "Verbal Ability",
    topics: ["Reading Comp", "Synonyms", "Grammar", "Para Jumbles"],
  },
  {
    icon: FileText,
    title: "Mock Tests",
    topics: ["Full-length", "Sectional", "Adaptive", "Previous papers"],
  },
];
            ...
            <div key={s.title} className="glass-card rounded-2xl p-6">
              ...
              <ul className="mt-3 grid grid-cols-2 gap-2">
                {s.topics.map((t) => (
                  <li
                    key={t}
                    className="text-sm px-3 py-1.5 rounded-lg bg-background/60 hover:bg-accent/10 transition-colors cursor-pointer"
                  >
                    {t}
                  </li>
                ))}
        ...
        <div className="mt-4 grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {COMPANIES.map((c) => (
            ...
            >
              ...
              <p className="mt-3 text-xs flex items-center gap-1 text-accent font-semibold">
