    ...
    (async () => {
      ...
      const greeting: UIMessage = {
        ...
        parts: [
          {
            type: "text",
            text: `Hi ${user.fullName.split(" ")[0] || "there"}! I'm TechLand AI — your career copilot. Ask me anything about your roadmap, resume, interviews, or career.`,
          },
        ],
      };
...
function AssistantChat({ initialMessages }: { initialMessages: UIMessage[] }) {
  ...
  return (
    <AppShell>
      <PageHeader
        title="TechLand AI"
        subtitle="Your 24/7 AI career coach — guidance, study plans, resume reviews and interview feedback."
      />
          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto p-6 space-y-4">
            {messages.map((m) => {
              ...
              return (
                ...
                >
                  <div
                    className={`size-9 rounded-full grid place-items-center shrink-0 ${m.role === "assistant" ? "bg-gradient-primary" : "bg-gradient-accent"}`}
                  >
                    {m.role === "assistant" ? (
                  ...
                  <div
                    className={`max-w-[75%] rounded-2xl px-4 py-3 text-sm whitespace-pre-wrap ${m.role === "assistant" ? "bg-background/70" : "bg-gradient-primary text-primary-foreground"}`}
                  >
            ...
            {status === "submitted" && (
              <div className="flex gap-3">
                ...
                <div className="rounded-2xl px-4 py-3 text-sm bg-background/70">
            ...
            <div className="mt-3 space-y-2">
              {SUGGESTIONS.map((s) => (
                <button
                  key={s}
                  onClick={() => send(s)}
                  className="block w-full text-left p-3 rounded-lg bg-background/60 hover:bg-accent/10 text-sm transition-colors"
                >
                  {s}
                </button>
              ))}
