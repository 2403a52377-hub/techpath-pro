...
function Leaderboard() {
  ...
  return (
    <AppShell>
      <PageHeader title="Leaderboard" subtitle="Compete with peers across India. XP, streaks, badges and gamified rewards." />

      <div className="grid lg:grid-cols-3 gap-6 mt-8">
        <div className="lg:col-span-2 glass-card rounded-2xl p-6">
          <h2 className="font-bold text-lg mb-4 flex items-center gap-2"><Trophy className="size-5 text-warning" /> Top this week</h2>
          <div className="space-y-2">
            {LEADERS.map((l, i) => (
              <div key={l.name} className={`flex items-center gap-4 p-3 rounded-xl ${i < 3 ? "bg-gradient-to-r from-warning/10 to-transparent" : "bg-background/50"}`}>
                <div className={`size-9 rounded-full grid place-items-center font-bold text-sm ${i === 0 ? "bg-warning text-warning-foreground" : i === 1 ? "bg-muted-foreground/30" : i === 2 ? "bg-orange-300/40" : "bg-muted"}`}>{i + 1}</div>
            ...
            {user && (
              <div className="flex items-center gap-4 p-3 rounded-xl bg-gradient-primary/15 border border-primary/30 mt-4">
                <div className="size-9 rounded-full bg-primary text-primary-foreground grid place-items-center font-bold text-sm">99</div>
                <div className="flex-1 min-w-0">
        ...
        <div className="space-y-4">
              ...
              {BADGES.map((b) => {
                ...
                return (
                  <div key={b.name} className="text-center">
                    <div className={`size-14 mx-auto rounded-2xl bg-gradient-to-br ${b.color} grid place-items-center shadow-md`}>
          ...
          <div className="glass-card rounded-2xl p-5">
            ...
            <ul className="mt-3 space-y-2 text-sm">
              <li className="flex justify-between"><span>7-day streak</span><span className="text-success font-semibold">✓ Unlocked</span></li>
              <li className="flex justify-between"><span>30-day streak</span><span className="text-muted-foreground">+500 XP</span></li>
              <li className="flex justify-between"><span>100-day streak</span><span className="text-muted-foreground">Free mentor session</span></li>
            </ul>
          </div>
        </div>
      </div>
