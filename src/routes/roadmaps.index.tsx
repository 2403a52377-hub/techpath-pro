...
type Roadmap = {
  ...
  domain_name: string; level: string; estimated_duration: string; icon: string | null;
};

function RoadmapsIndex() {
  const [roadmaps, setRoadmaps] = useState<Roadmap[] | null>(null);
  useEffect(() => {
    supabase.from("roadmaps").select("*").order("sort_order").then(({ data }) => setRoadmaps(data ?? []));
  }, []);
  ...
  return (
    <AppShell>
      ...
      {!roadmaps && <div className="grid place-items-center py-16"><Loader2 className="size-6 animate-spin text-primary" /></div>}
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5 mt-8">
        {roadmaps?.map((r) => (
          <Link key={r.id} to="/roadmaps/$slug" params={{ slug: r.slug }} className="group glass-card rounded-2xl p-6 hover:shadow-elegant hover:-translate-y-1 transition-all">
            ...
            <div className="mt-4 flex items-center justify-between text-xs text-muted-foreground">
              <span className="flex items-center gap-1"><Clock className="size-3.5" />{r.estimated_duration}</span>
