...
function CompaniesIndex() {
  return (
    <AppShell>
      <PageHeader title="Company Preparation Tracks" subtitle="Dedicated prep pages with interview process, required skills, FAQs, and a roadmap for each company." />
      <div className="mt-8 grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
        {COMPANIES.map((c) => (
          <Link key={c.slug} to="/companies/$slug" params={{ slug: c.slug }}
            ...
            <p className="text-sm text-muted-foreground">{c.role} • {c.ctc}</p>
            <p className="mt-4 text-sm font-semibold text-accent flex items-center gap-1">Open track <ArrowRight className="size-4" /></p>
          </Link>
        ))}
      </div>
    </AppShell>
